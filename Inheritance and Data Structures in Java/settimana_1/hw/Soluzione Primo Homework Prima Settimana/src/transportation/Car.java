...
public class Car extends Vehicle {
	...
	public void refuel(int amountOfGas) {
		if((amountOfGas + super.getGasRemained()) > this.maxGasAmountInTank) {
			super.gasRemained=this.maxGasAmountInTank;
		}
		else {
			super.gasRemained = super.gasRemained + amountOfGas;
		}
	}
	...
	public void run(int hour) {
		if(this.gasRemained <= 0) {
			...
			//devo calcolare il carburante consumato durante la corsa
			this.gasRemained = this.gasRemained - (hour * this.gasConsumedPerHour);
			if(this.gasRemained <= 0) {
				System.out.println("Oops, gas out! Please add fuel!");
				this.gasRemained = 0;
			}
			this.totalGasConsumed = this.totalGasConsumed + (hour * this.gasConsumedPerHour);
			if((hour * this.gasConsumedPerHour) > this.gasRemained) {
				this.totalGasConsumed = this.totalGasConsumed + this.gasRemained;
			}
		}
	}
}
